...
function makeLc(overrides: Partial<LeadCentresStyleOptions> = {}): Required<LeadCentresStyleOptions> {
  return { ...DEFAULT_STYLE.leadCentres as Required<LeadCentresStyleOptions>, ...overrides };
}
...
describe('normalizeLeadCentresInput', () => {
  it('normalizes a valid entry with lat/lon', () => {
    const result = normalizeLeadCentresInput([{ lat: 51.5, lon: -0.1, label: 'GOSH' }]);
  ...
  it('accepts latitude/longitude aliases', () => {
    const result = normalizeLeadCentresInput([{ latitude: 53.0, longitude: -1.5, label: 'Sheffield' }]);
    expect(result).toHaveLength(1);
  ...
  it('passes through extra fields as feature properties', () => {
    const result = normalizeLeadCentresInput([{
      ...
      total_patients: 312, median_hba1c: 58,
    }]);
  ...
  it('excludes coordinate alias keys from properties', () => {
    const result = normalizeLeadCentresInput([{ lat: 51.5, lon: -0.1, latitude: 51.5, longitude: -0.1, lng: -0.1 }]);
  ...
  it('skips entries with missing coordinates and calls onWarning', () => {
    ...
    const result = normalizeLeadCentresInput([{ label: 'No coords' }], {}, onWarning);
    expect(result).toHaveLength(0);
    expect(onWarning).toHaveBeenCalledWith(expect.objectContaining({ code: 'MISSING_LEAD_CENTRE_COORDS' }));
  });

  it('skips entries with invalid coordinates and calls onWarning', () => {
    const onWarning = vi.fn();
    const result = normalizeLeadCentresInput([{ lat: 999, lon: -0.1 }], {}, onWarning);
    expect(result).toHaveLength(0);
    expect(onWarning).toHaveBeenCalledWith(expect.objectContaining({ code: 'INVALID_LEAD_CENTRE_COORDS' }));
  });
  ...
  it('skips non-object entries and calls onWarning', () => {
    ...
    expect(result).toHaveLength(0);
    expect(onWarning).toHaveBeenCalledWith(expect.objectContaining({ code: 'INVALID_LEAD_CENTRE_ENTRY' }));
  });
...
describe('buildBubbleSizeExpression', () => {
  ...
  it('returns an interpolate expression when range is valid', () => {
    const lc = makeLc({ sizeField: 'count', minRadius: 8, maxRadius: 40, defaultRadius: 12 });
    const expr = buildBubbleSizeExpression(lc, { sizeMin: 0, sizeMax: 500 }) as unknown[];
  ...
  it('uses the configured sizeField in the expression', () => {
    const lc = makeLc({ sizeField: 'patient_count', minRadius: 5, maxRadius: 30, defaultRadius: 10 });
    const expr = buildBubbleSizeExpression(lc, { sizeMin: 10, sizeMax: 200 }) as unknown[];
...
describe('buildBubbleColorExpression', () => {
  describe('continuous mode', () => {
    it('returns first stop color when colorMin === colorMax', () => {
      const lc = makeLc({ colorMode: 'continuous', colorScale: ['#2166ac', '#f7f7f7', '#d6604d'], colorField: 'hba1c' });
      const expr = buildBubbleColorExpression(lc, { colorMin: 58, colorMax: 58 });
      expect(expr).toBe('#2166ac');
    });

    it('returns an interpolate expression for valid range', () => {
      const lc = makeLc({ colorMode: 'continuous', colorScale: ['#2166ac', '#f7f7f7', '#d6604d'], colorField: 'hba1c' });
      const expr = buildBubbleColorExpression(lc, { colorMin: 48, colorMax: 86 }) as unknown[];
      expect(Array.isArray(expr)).toBe(true);
      expect(expr[0]).toBe('interpolate');
    });

    it('includes the colour stops in the expression', () => {
      const lc = makeLc({ colorMode: 'continuous', colorScale: ['#aa0000', '#ffffff', '#0000aa'], colorField: 'hba1c' });
      const expr = buildBubbleColorExpression(lc, { colorMin: 0, colorMax: 100 }) as unknown[];
  ...
  describe('categorical mode', () => {
    it('returns colorFallback when colorByCategory is empty', () => {
      const lc = makeLc({ colorMode: 'categorical', colorByCategory: {}, colorFallback: '#999999', colorField: 'type' });
    ...
    it('returns a match expression when colorByCategory has entries', () => {
      ...
      const expr = buildBubbleColorExpression(lc, { colorMin: 0, colorMax: 0 }) as unknown[];
      expect(Array.isArray(expr)).toBe(true);
